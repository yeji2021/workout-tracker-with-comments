...
### 5-1. 인스타그램 (기본 경로 — 실사용의 대부분)

URL 형태는 `/reel/{코드}/`, `/reels/{코드}/`, `/p/{코드}/` 셋이다. 그대로 열면 된다.
`/p/` 는 릴스가 아니라 사진·캐러셀 게시물일 수 있다 — 이 경우 영상이 아니라 **이미지들을
넘겨 가며** 보는 것이 6절의 폴백에 해당한다.

읽을 것을 정보량 순으로:

1. **캡션 본문** — 접혀 있으면 "더 보기"/"more" 를 눌러 펼친다. 루틴이 캡션에 있으면 보통
   여기 전부 있다.
2. **첫 댓글 / 고정 댓글** — 캡션 글자 수 때문에 루틴을 첫 댓글로 빼는 계정이 흔하다.
   캡션이 짧고 내용이 없으면 반드시 확인한다.
3. **해시태그 덩어리** — 버린다. `#운동 #헬스` 는 정보가 아니다. 다만 `#3분할` 처럼
   루틴 성격을 알려주는 건 `source_summary` 에 참고로 쓸 수 있다.

**유튜브와 결정적으로 다른 점**: 릴스는 루틴이 캡션이 아니라 **영상 화면의 오버레이 자막에만**
있는 경우가 오히려 많다. 캡션이 `"오늘 등 루틴 🔥 저장 필수"` 수준이면 그건 정보가 없는
것이니 붙들고 있지 말고 바로 6절 캡처 폴백으로 넘어가라. 인스타에서는 캡처가 예외 경로가
아니라 **정상 경로**다.

즉시 실패 처리(캡처 폴백으로 넘어가지 마라, 9절 문구 사용):

- 로그인 벽이 뜬다 → 브라우저 세션이 풀린 것. 사용자에게 알리고 그 건은 실패로 두지 말고
  **큐에 남긴 채**(processing 해제) 중단한다. 로그인만 다시 하면 되는 상황이라 실패로
  못박으면 사용자가 재등록해야 한다.
- 비공개 계정 / 팔로우 요청 화면 → 실패. 관리자가 팔로우하지 않으면 볼 수 없다.
- 삭제됨 / 페이지 없음 → 실패.
...
**인스타 릴스는 이 경로로 오는 게 정상이다** — 캡션이 부실하다고 실패로 넘기지 마라.
`/p/` 캐러셀 게시물이면 재생 대신 이미지를 한 장씩 넘기며 같은 방식으로 읽는다.
